...
function validateChallenges(challenges, roles, sentencesPerRole) {
  ...
  const conflictRoleIds = new Set(roles.filter(role => ['backend', 'frontend', 'qa'].includes(role.id)).map(role => role.id));
  ...
  for (const item of challenges) {
    ...
    const isConflict = item?.is_conflict === true;
    ...
    valid.push({ role, content, isConflict });
  }
  if (roles.some(role => (counts.get(role.id) || 0) !== sentencesPerRole)) return [];
  if (conflictRoleIds.size > 0 && valid.filter(item => item.isConflict).length !== 1) return [];
  if (valid.some(item => !conflictRoleIds.has(item.role.id) && item.isConflict)) return [];
...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      ...
      const url = required(process.env.SUPABASE_URL, 'SUPABASE_URL');
      const response = await fetch(`${url}/rest/v1/document_challenge_runs?source_document_id=eq.${encodeURIComponent(sourceDocumentId)}&select=id,challenges,created_at&order=created_at.desc&limit=1`, { headers: supabaseHeaders() });
      if (!response.ok) throw new Error(`读取模拟质疑失败：${await response.text()}`);
      const [run] = await response.json();
      return json(res, 200, { run: run || null });
    }
    ...
    const relatedContext = relatedDocuments.length
      ...
      : '无';
    const conflictRoles = selectedRoles.filter(role => ['backend', 'frontend', 'qa'].includes(role.id));
    const prompt = `你是企业产品研发协作中的审阅伙伴。请根据资料，让每个指定角色从自己的专业视角提出能帮助作者补全文档的疑问。\n\n原始文档标题：${sourceDocument.title}\n原始文档：\n${sourceDocument.content}\n\n关联资料：\n${relatedContext}\n\n参与角色与关注点：\n${selectedRoles.map(role => `- ${role.id}：${role.name}。${ROLE_LENSES[role.id] || '从该角色的日常工作视角追问流程、边界和验收。'}`).join('\n')}\n\n只返回合法 JSON，不要使用 Markdown 或代码块：\n{"challenges":[{"role_id":"角色 id","content":"一句口语化、具体的追问","is_conflict":false}]}\n\n规则：\n- 每个角色恰好 ${sentencesPerRole} 条，合计 ${selectedRoles.length * sentencesPerRole} 条。\n- 只提出资料尚未明确、但与该角色职责直接相关的问题；不要杜撰事实或给出答案。\n- ${conflictRoles.length ? `在 ${conflictRoles.map(role => role.name).join('、')} 的全部问题中，合计且仅能有 1 条 is_conflict 为 true；由最适合发现该问题的角色提出，其余问题都必须为 false。` : '所有问题的 is_conflict 都必须为 false。'} 重点质疑要从原始文档的不同段落、或原始文档与关联资料之间，找出两种规则、口径、流程或状态处理可能无法同时成立的地方，像熟悉现网的同事在确认“新方案接入后原有处理会怎么衔接”。问题正文不要出现“冲突”“矛盾”“现有逻辑冲突”等标签词，不要凭空编造系统能力；请用具体对象、条件和后果自然地提出追问。\n- 像同事在评审会上自然追问，使用具体的“如果……要怎么处理呢？”、“这个……由谁来确认呢？”一类句式。示例：前端角色可问“如果用户更改了手机号，要怎么处理呢？”。
...
- 若资料已经明确，请追问与该内容紧邻的异常路径、操作后的反馈、责任归属或边界条件。`;
    ...
    const run = await saveChallengeRun({
      ...
      challenges: challenges.map(item => ({ role_id: item.role.id, role_name: item.role.name, content: item.content, is_conflict: item.isConflict })),
      model,
    });
