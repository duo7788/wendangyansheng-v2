...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(`${url}/rest/v1/document_derivations?source_document_id=eq.${encodeURIComponent(sourceDocumentId)}&select=role_id,content,related_document_ids,updated_at`, {
      headers,
    });
